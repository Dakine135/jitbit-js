...
class jitBit {
   ...
   * @date November 15 2024 11:11 am -0500
   * @param {*} {baseUrl, username, password, token}
   * @memberof jitBit
   */
  ...
  async #makeRequest({ endpoint, method, params, encoding = null, postData }) {
    ...
    return new Promise((resolve, reject) => {
      ...
      //     encoding,
      //   },
      //   (requestError, res, body) => {
      //     const statusCode = res?.statusCode;
      //     if (requestError) {
      //       console.error(`Error in jitbitApi#${endpoint}`, requestError);
      //       reject(requestError);
      //       return;
      //     }
      //     if (statusCode != 200) {
      //       reject(new Error(endpoint + 'received Status Code:' + statusCode + ' ' + body));
      //       return;
      //     }
