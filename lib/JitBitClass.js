...
function bufferConvertCatch(input) {
  try {
...
class jitBit {
  ...
  async #makeRequest({ endpoint, method, params = {}, encoding = null, postData, ignoreCache = false, retryTimeout = 500 }) {
    ...
    const fullUrlObject = new URL(this.#baseUrl + endpoint);
    Object.entries(params).forEach(([key, value]) => {
      if (Array.isArray(value)) value.forEach((x) => fullUrlObject.searchParams.append(key, x));
      else fullUrlObject.searchParams.append(key, value);
    });
    const fullUrlString = fullUrlObject.toString();

    if (!this.disableCaching && !ignoreCache && method === 'GET' && fullUrlString in this.#cache) return this.#cache[fullUrlString].data;
    ...
    if (this.DEBUG) console.log('headers :>> ', headers);
    if (this.DEBUG) console.log('postData :>> ', postData);

    const resultPromise = new Promise((resolve, reject) => {
      let statusCode,
        statusMessage,
        responseHeaders = null;
      const request = http.request(fullUrlString, { method, headers }, (response) => {
        ...
        response.on('end', () => {
          ...
          const bodyParsed = parseJsonCatch(body) ?? bufferConvertCatch(body) ?? body;
          if (statusCode != 200) error = new Error(bodyParsed ?? statusMessage ?? statusCode);
          resolve({ statusCode, statusMessage, responseHeaders, data: bodyParsed, error });
        });
      });
    ...
    await resultPromise.then((result) => {
      ...
      if (!this.disableCaching && method === 'GET' && !result.error && results.statusCode != 429) {
        this.#cache[fullUrlString] = { data: result, ttl: this.cacheTimeToLive };
  ...
  async deleteTicket(params = {}) {
    ...
    const result = await this.#makeRequest({ endpoint, method, postData: params });
    if (result && result.error) throw result.error;
    if (result && result.statusCode == 200) return result.data;
    else return null;
  }
  ...
  async createTicket(params = {}) {
    if (this.DEBUG) console.log('createTicket params START :>> ', params);
    const endpoint = '/ticket';
    const method = 'POST';
    ...
    if (this.DEBUG) console.log('createTicket params AFTER PROCESSING :>> ', params);
    ...
    if (!params.dryRun) {
      ...
      if (result && result.statusCode == 200) {
        //the jitbit api doesn't support due date on creation, but it does on update
        if (params.dueDate) await this.updateTicket({ ticketId, dueDate: params.dueDate });
        return ticketId;
      }
    }
    return null;
  ...
  async updateTicket(params = {}) {
    ...
    const result = await this.#makeRequest({ endpoint, method, postData: params });
    if (result && result.error) throw result.error;
    if (result && result.statusCode == 200) return true;
    else return false;
  }

  /**
   * @description Add a comment to a ticket
   * @date January 15 2025 11:46 pm -0500
   * @param {Object} params
   * @param {int} params.id - Ticket ID
   * @param {int} params.ticketId - (Alias) Ticket id
   * @param {int} params.issueId - (Alias) Ticket id
   ...
   * @param {string} params.comment - (Alias) Comment body
   * @param {boolean} [params.forTechsOnly=false] - (optional) If this comment is for techs only. Default: false
   * @param {boolean} [params.isSystem=false] - (optional) If this is a "system" comment (shown in grey). Default: false
   * @param {integer[]} [params.recipientIds=null] - (optional) A list of subscriber user IDs to send the comment notification to. By default it is sent to all subscribers. Default: null
   * @param {integer} [params.fromUserId] - (optional) the userId from which to post a comment from, required Administrator role
   * @return {Boolean} true if status is 200, otherwise false
   */
  async addComment(params = {}) {
    ...
    if (params.body == null) params.body = params.comment;
    if (params.body == null) throw new Error('Missing comment body');
