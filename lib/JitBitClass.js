...
class jitBit {
   ...
   * @param {int[]} params.statusId - (optional) Filter by status(es), int or an Array of ints
  ...
  async createTicket(params = {}) {
    ...
    //todo if userId lookup userId and error if not found
    //todo lookup by email userID option
    //todo lookup by username userID option
  ...
  async getUser(params = {}) {
    let endpoint = null;
    const method = 'GET';
    if (params.username) endpoint = '/UserByUsername';
    if (params.email) endpoint = '/UserByEmail';
    if (params.userId) endpoint = '/User';

    if (endpoint == null) throw new Error('Missing user lookup parameter (userId, email, or username)');
