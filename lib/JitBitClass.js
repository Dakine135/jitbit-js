...
class jitBit {
    ...
    const resultPromise = new Promise((resolve, reject) => {
      ...
      const request = http.request(fullUrlString, { method, headers }, (response) => {
        ...
        response.on('end', () => {
          ...
          let error;
          if (statusCode == 404) error: statusMessage;
          try {
            const bodyParsed = JSON.parse(body);
            resolve({ statusCode, statusMessage, responseHeaders, data: bodyParsed, error });
          } catch (error) {
            resolve({ statusCode, statusMessage, responseHeaders, data: body, error });
          }
        });
      });
  ...
  async createTicket(params = {}) {
    ...
    if (!params.dryRun) {
      ...
      if (result && result.statusCode == 200) return result.data;
      if (result && result.statusCode == 404 && result.data) {
        //buffer to string parse
