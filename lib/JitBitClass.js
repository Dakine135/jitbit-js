...
class jitBit {
  ...
  async createTicket(params = {}) {
    ...
    if (!params.dryRun) {
      let restOfBody;
      let sendBodyAsUpdate = false;
      if (params.body && params.body.length > 1600) {
        restOfBody = params.body.substring(1599);
        params.body = params.body.substring(0, 1599); //Jitbit API only supports 1600 characters
        sendBodyAsUpdate = true;
      }
      const result = await this.#makeRequest({ endpoint, method, params });
      ...
      if (result && result.statusCode == 200) {
        ...
        if (sendBodyAsUpdate && body) {
          console.log('sendBodyAsUpdate', ticketId, restOfBody);
          await this.addComment({ ticketId, comment: restOfBody });
        }
        return ticketId;
      }
    }
