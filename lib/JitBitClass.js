...
class jitBit {
  ...
  async createTicket(params = {}) {
    ...
    if (params.userId || params.userUsername || params.userEmail) {
      ...
      if (!user) throw new Error('No User Found for userId/username/email "on-behalf"');
    }
    if (params.assignedToUserId || params.assignedToUserUsername || params.assignedToUserEmail) {
      ...
      if (!assignedToUser) throw new Error('No User Found with userId/username/email to assign ticket to');
    }
